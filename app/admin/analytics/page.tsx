...
export default function AdminAnalyticsPage() {
  ...
  const fetchAnalytics = async (useCache: boolean = true) => {
    try {
      ...
      const url = `/api/admin/analytics/overview?${params}`;
      const cacheKey = CACHE_KEYS.ANALYTICS(timeRange);
      
      let data;
      if (useCache) {
        data = await adminFetchCached(url, cacheKey, CACHE_TTL.ANALYTICS);
      } else {
